		...
		<div class = "container">
			...
			<a href="" class="btn btn-info navbar-btn navbar-right">Sign In</a> -->
				...
				<ul class="nav navbar-nav">
								...
								<li><a href="index.html#feedback">Feedback</a>
								</ul>
...
<!-----------------------------COMPANY BANNER SECTION BELOW---------------------->	
	...
	<center><h1> Welcome! View All Appointments</h1></center>
...
$password = $_POST['password'];
 ...
 $result = mysql_query( "SELECT ID FROM registeredoctors WHERE username LIKE '$username' AND passw LIKE '$password' ", $dbc );
//$result = mysql_fetch_array($id);
$row = mysql_fetch_row($result);
$id = $row[0];

$retval  = mysql_query( "SELECT * FROM appointmenttable WHERE doctorid LIKE '$id'", $dbc );
...
while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
{
	...
	echo "<tr><td>".$row["patientfullname"]."</td><td>".$row["contactnumber"]."</td><td>".$row["message"]."</td></tr>";
